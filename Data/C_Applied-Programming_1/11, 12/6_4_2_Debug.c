...
int Call_num = 0; // calculateCentroid 関数 の呼び出し回数
...
void calculateCentroid()
{
    ...
    for (int i = 0; i < k; i++)
    {
        ...
        printf("y_sum: %f, x_sum: %f, group_count: %f\n", y_sum, x_sum, group_count);
    }
}
...
void assignDataToNearestCentroid()
{
    for (int i = 0; i < NUM_DATA; i++)
    {
        ...
        printf("group: %d, min_distance: %f\n", nearest, min_distance);
        printf("group[%d] = %d → ", i, group[i]);
        group[i] = nearest;
        printf("%d\n", group[i]);
        printf("--------------------------------------\n");
    }
