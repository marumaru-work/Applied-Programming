...
int main(void)
{
    ...
    char outputFile[] = "./result_7_2_3.bmp";  // 出力画像のファイル名

    // 画像の読み込み
    readBmp(inputFile, imageIn);

    // 実装する場所
    char imagesave[Y_SIZE][X_SIZE];
    for (int y = 0; y < Y_SIZE; y++)
    {
        for (int x = 0; x < X_SIZE; x++)
        {
            imagesave[y][x] = imageIn[y][x];
        }
    }
